...
import com.zjzmjr.web.mvc.annotation.AccessHeartBeat;
...
public class HeartBeatAspect implements Ordered{
     ...
     * @throws Throwable
     */
    @Around("@annotation(com.zjzmjr.web.mvc.annotation.AccessHeartBeat)")
    public Object invokeAround(ProceedingJoinPoint jp) throws Throwable {
        ...
        try {
            ...
            // 设置访问间隔数
            // ip地址的访问限制
            String ip = getUserIp(req, 1);
            if (StringUtils.isNotBlank(ip)) {
                String hbIpKey = BEAT_HB_IP_KEY.replace("{ip}", ip);
                Integer count = JedisPull.getObject(empty, hbIpKey, Integer.class);
                if (count != null && count >= BEAT_IP_HEART_COUNT) {
                    throw new IllegalArgumentException("该IP访问过于频繁");
                }
                JedisPull.setObject(empty, hbIpKey, count == null ? 1 : count + 1, beat.interval());
            }

            if (StringUtils.isNotBlank(mobile)) {
                String hbMobileKey = BEAT_HB_MOBILE_KEY.replace("{mobile}", mobile);
                Integer count = JedisPull.getObject(empty, hbMobileKey, Integer.class);
                if (count != null && count >= beatCount) {
                    throw new IllegalArgumentException("该手机访问过于频繁");
                }
                JedisPull.setObject(empty, hbMobileKey, count == null ? 1 : count + 1, beat.interval());
            }
	 ...
	 * @return
	 */
	private String getUserIp(HttpServletRequest request,int proxy){
		if(request!=null){
			if(proxy>=1){
